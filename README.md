...
### Webots exits immediately with a Python error

Read the first traceback in the Webots console. Common causes are:

- the wrong script was copied to `robot_controller.py`;
- a required package is missing from the Python environment used by Webots;
- `Webots/model_export.json` is missing for plaintext or CKKS evaluation.

### Dataset size is not 192,680

Check that `data_collector.py` contains:

```python
N_ROLLOUTS_PER_TRAJECTORY = 40
```

and confirm that all eight trajectories completed.

### `train.py` cannot find the dataset

Make sure the following file exists in the repository root:

```text
webots_dataset.npz
```

and run `python train.py` from the repository root.

### Server cannot find the model
...
### Webots client cannot find the model
...
For the same-machine setup, confirm:
...
and start `server.py` before running Webots.

### CKKS mode reports that TenSEAL is missing

TenSEAL must be installed in both:

- the Python environment running `server.py`;
- the Python environment used by the Webots controller.
